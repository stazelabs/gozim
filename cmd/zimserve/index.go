...
<select id="search-zim"`)
	...
	for _, slug := range lib.slugs {
		e := lib.archives[slug]
		fmt.Fprintf(w, `<option value="%s">%s</option>`, html.EscapeString(slug), html.EscapeString(e.title))
	}
	fmt.Fprint(w, `</select>
...
<table><colgroup><col class="title"><col><col class="date"><col class="entries"></colgroup><thead><tr>
