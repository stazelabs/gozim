...
import (
	...
	"github.com/stazelabs/gozim/docs"
...
func init() {
	...
	if err := md.Convert([]byte(docs.ZimserveMD), &buf); err != nil {
		renderedDocs = "<pre>" + html.EscapeString(docs.ZimserveMD) + "</pre>"
		return
	}
...
<html><head><meta charset="utf-8"><title>Documentation — zimserve</title>
` + faviconLink + `
<style>
...
<div class="nav" style="margin-bottom:16px"><a href="/">← Back to Library</a></div>
`)
	fmt.Fprint(w, renderedDocs)
	fmt.Fprint(w, `<div class="nav"><a href="/">← Back to Library</a></div>`)
